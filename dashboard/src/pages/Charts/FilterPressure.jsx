// src/pages/Pressure/FilterPressurePage.jsx
...
/* ===== Page: Filter Pressure ===== */
export default function FilterPressurePage() {
  ...
  const liveFilter = useSelector((s) => s.live?.pressure_filter); // 👈 Redux live data
  ...
  const loadCache = useCallback(() => {
    try {
      const raw = localStorage.getItem(`hist.press.filter.${keyRef.current}`);
  ...
  const saveCache = useCallback((rows) => {
    try {
      localStorage.setItem(
        `hist.press.filter.${keyRef.current}`,
        JSON.stringify(normalize(rows))
      );
    } catch {}
  }, []);

  // Đổi ngày → nạp cache mới của ngày đó
  ...
  // 1️⃣ Khôi phục cache ngay khi vào trang
  useEffect(() => {
    const restored = loadCache();
    setSeries(restored);
  }, [loadCache]);
  ...
  const fetchHistoryToday = useCallback(async () => {
    try {
      setLoading(true);
      const url = `${apiBase}/api/sensor/pressure/filter/history?bucketSec=60`;
      const res = await fetch(url);
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || res.statusText);

      const rawArr =
        Array.isArray(json) ? json : json?.pressure || json?.filter || json?.data;
      ...
      dispatch(
        fetchDataFailure(err?.message ?? "get filter pressure history error")
      );
    } finally {
      setLoading(false);
    }
  ...
  // 3️⃣ Khi Redux liveFilter thay đổi → thêm điểm vào series
  useEffect(() => {
    if (Number.isFinite(liveFilter)) append(liveFilter);
  }, [liveFilter, append]);
  ...
  const currentStr = useMemo(() => {
    if (!Number.isFinite(liveFilter)) return "--";
    return liveFilter.toFixed(1);
  }, [liveFilter]);

  return (
    <div className="w-full px-3 md:px-6">
      <div className="text-center">
        <Divider label={t("Filter Pressure")} />
      </div>
      ...
      <div className="bg-white/70 backdrop-blur rounded-2xl shadow-sm p-4 md:p-6">
        ...
        <div className="mt-3">
          <PressureChart
            data={series}
            title={t("Filter Pressure Chart (Pa)")}
            unit="Pa"
            min={-100}
            max={100}
          />
