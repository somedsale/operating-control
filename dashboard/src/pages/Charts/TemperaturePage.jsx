...
const normalize = (arr) => {
  ...
  const bySec = new Map(); // key theo giây
  for (const r of rows) {
...
export default function TemperaturePage() {
  ...
  // 1) Khôi phục cache ngay khi vào trang
  useEffect(() => {
    const restored = loadCache();
    setSeries(restored);
  ...
  const fetchHistoryToday = useCallback(async () => {
    try {
      ...
      const rawArr = Array.isArray(json) ? json : json?.temp;
  ...
  const fetchCurrent = useCallback(async () => {
    try {
      ...
      dispatch(fetchDataFailure(err?.message ?? "getTemperature error"));
    }
  }, [append, dispatch, rolloverDayIfNeeded]);

  // Chu trình tải dữ liệu
  useEffect(() => {
    // Lần đầu: hợp nhất cache + lịch sử API, rồi bắt đầu poll hiện tại
    fetchHistoryToday().then(fetchCurrent);

    // Cập nhật hiện tại mỗi 10s
    const id = setInterval(fetchCurrent, 10000);

    // Làm tươi lịch sử mỗi 5 phút
      ...
      <div className="bg-white/70 backdrop-blur rounded-2xl shadow-sm p-4 md:p-6">
        <div className="flex items-baseline justify-between">
          <div className="text-slate-600">
            {t("Current")}: <span className="font-semibold">{currentStr}°C</span>
          </div>
